...
<%--    </a>--%>
    ...
    <table class="table table-striped table-bordered " id="employeeTable">
        ...
        <c:forEach varStatus="status" var="facility" items="${employeeList}">
            <tr>
                <td class="text-center">${status.count}</td>
                <td>${facility.nameEmployee}</td>
...
<%--                </c:if>--%>
                <td class="text-center">${facility.idCard}</td>
                <td class="text-center">${facility.salary}</td>
                <td class="text-center">${facility.phoneNumber}</td>
                <td>${facility.email}</td>
                <td>${facility.address}</td>
                <c:forEach var="position" items="${positionList}">
                    <c:if test="${position.positionId == facility.position}">
                        <td class="text-center">${position.name}</td>
                    </c:if>
                </c:forEach>
                <c:forEach var="educationDegree" items="${educationDegreeList}">
                    <c:if test="${educationDegree.id == facility.educationDegreeId}">
                        <td class="text-center">${educationDegree.name}</td>
                    </c:if>
                </c:forEach>
                <c:forEach var="division" items="${divisionList}">
                    <c:if test="${division.id == facility.divisionId}">
                        <td class="text-center">${division.name}</td>
                    </c:if>
                </c:forEach>
                <td class="text-center"><a href="/employee?action=edit&id=${facility.id}">
